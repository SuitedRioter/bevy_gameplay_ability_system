...
use super::manager::{
    ActiveWhileActiveCues, GameplayCueEvent, GameplayCueManager, GameplayCueParameters,
    StaticCueHandlers,
};
use super::notify::{
...
/// currently active. It queries all entities with ActiveWhileActiveCues
/// and invokes the corresponding handlers.
pub fn update_while_active_cues_system(
    handlers: Res<StaticCueHandlers>,
    mut commands: Commands,
    mut query: Query<(Entity, &mut ActiveWhileActiveCues)>,
) {
    for (entity, mut active_cues) in query.iter_mut() {
        // Collect tags to remove (if handler is missing)
        let mut tags_to_remove = Vec::new();

        for (cue_tag, params) in active_cues.cues.iter() {
            if let Some(handler) = handlers.get(cue_tag) {
                // Call while_active on the handler
                handler.while_active(entity, params, &mut commands);
            } else {
                // Handler not found, mark for removal
                warn!(
                    "WhileActive cue handler not found for tag: {:?}",
                    cue_tag
                );
                tags_to_remove.push(cue_tag.clone());
            }
        }

        // Remove missing handlers
        for tag in tags_to_remove {
            active_cues.remove(&tag);
        }

        // If no more active cues, remove the component
        if active_cues.is_empty() {
            commands.entity(entity).remove::<ActiveWhileActiveCues>();
        }
    }
}
...
mod while_active_tests {
    ...
    fn test_while_active_cues_update() {
        ...
        app.add_systems(Update, update_while_active_cues_system);
